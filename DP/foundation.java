...
public class foundation {
    ...
    // ============
    // THRoUGH STRIVER's code

    public static int BSS_K_Transactions_MEMO(int[] prices, int idx, int buy, int k, Integer[][][] dp) {
        ...
        int profit = 0;
        if (buy == 1) {
            profit = Math.max(-prices[idx] + BSS_K_Transactions_MEMO(prices, idx + 1, 0, k, dp),
                    BSS_K_Transactions_MEMO(prices, idx + 1, 1, k, dp));
        } else {
            profit = Math.max(prices[idx] + BSS_K_Transactions_MEMO(prices, idx + 1, 1, k - 1, dp),
                    BSS_K_Transactions_MEMO(prices, idx + 1, 0, k, dp));
        }
        return dp[idx][buy][k] = profit;
    }

    public static int BSS_K_Transactions_TAB(int[] prices, int k) {
